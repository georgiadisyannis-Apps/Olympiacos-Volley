...
st.set_page_config(page_title="Volley Tactical Pro v64", layout="centered")

# --- CSS v64: Responsive UI & Top Padding Fix ---
st.markdown("""
<style>
    /* 1. Προσθήκη κενού στην κορυφή για να μην κρύβονται οι τίτλοι */
    .block-container { 
    ...
    .stAlert { margin-top: 10px; border-radius: 8px; }
...
if st.session_state.page == "database":
    ...
    # Εμφάνιση Επιβεβαίωσης αν υπάρχει
    ...
    with st.expander("📝 Φόρμα Ομάδας", expanded=st.session_state.edit_team is not None):
        ...
        c1, c2 = st.columns(2)
        p = c1.number_input("Π (Πασαδόρος)", value=vals[0])
        k1 = c2.number_input("Κ1 (Κεντρικός)", value=vals[1])
        a2 = c1.number_input("Α2 (Ακραίος)", value=vals[2])
        d = c2.number_input("Δ (Διαγώνιος)", value=vals[3])
        k2 = c1.number_input("Κ2 (Κεντρικός)", value=vals[4])
        a1 = c2.number_input("Α1 (Ακραίος)", value=vals[5])
        ...
        if st.button("✅ Αποθήκευση", use_container_width=True):
            if name:
                ...
                st.session_state.confirm_msg = f"Η ομάδα '{name}' ενημερώθηκε!"
                st.rerun()

    # Λίστα Διαχείρισης
    st.markdown("### 📋 Οι Ομάδες μου")
    for t_name in list(st.session_state.teams_db.keys()):
        with st.container():
            ...
            if col_d.button("🗑️ Delete", key=f"d_{t_name}"):
                ...
                st.session_state.confirm_msg = "Η ομάδα διαγράφηκε."
                st.rerun()
